...
class Bridges2ArtifactTests(unittest.TestCase):
        ...
        for config_path in config_paths:
            with self.subTest(config_path=config_path.name):
                ...
                generated_validation_configs = {
                    ...
                    "base_diffusion_2to1_clean_bridges2.yaml",
                    "base_diffusion_2to1_lambdasyn0025_clean_bridges2.yaml",
                }
    ...
    def test_configs_do_not_use_voxceleb2_full_speaker_count(self):
        config_paths = sorted((ROOT / "configs").glob("*.yaml"))
        ...
        for config_path in config_paths:
            with self.subTest(config_path=config_path.name):
                ...
                self.assertNotIn("num_spk: 7323", source)
    ...
    def test_shared_slurm_body_trains_then_tests(self):
        ...
        self.assertIn("TEST_TRIAL_PATH=\"${CAARMA_TEST_TRIAL_PATH:-${REPO_ROOT}/data/veri_test.txt}\"", source)
    ...
    def test_base_amsoftmax_vox1_config_matches_voxceleb1_baseline_knobs(self):
        source = (ROOT / "configs" / "base_amsoftmax_vox1_bridges2.yaml").read_text()

        self.assertIn("VoxCeleb1-only MFA-Conformer config", source)
        self.assertIn("experiment_type: base", source)
        self.assertIn("condition_attribute: none", source)
        self.assertIn("synthetic_strategy: none", source)
        self.assertIn("adversarial_enabled: false", source)
        self.assertIn("criterion: AMSoftmax", source)
        self.assertIn("dataset: null", source)
        self.assertIn("active_split: train", source)
        self.assertIn("derive_num_spk: true", source)
        self.assertIn("init_lr: 0.001", source)
        self.assertIn("batch_size: 200", source)
        self.assertIn("num_workers: 4", source)
        self.assertIn("num_spk: 942", source)
        self.assertIn("trial_path: null", source)
        self.assertIn("save_dir: ${PROJECT}/caarma-output/base-amsoftmax-vox1-clean", source)
        self.assertIn("title: caarma_base_amsoftmax_vox1_clean", source)
        self.assertNotIn("active_split: all", source)
        self.assertNotIn("data/veri_test.txt", source)
        self.assertNotIn("diffusion_fake_fraction", source)
        self.assertNotIn("lambda_syn", source)
        self.assertNotIn("voxceleb_full.csv", source)
        self.assertNotIn("7323", source)
        self.assertNotIn("caarma-gender", source)
    ...
    def test_base_diffusion_2to1_clean_config_sets_half_fake_fraction(self):
        source = (ROOT / "configs" / "base_diffusion_2to1_clean_bridges2.yaml").read_text()

        self.assertIn("2:1 real-to-fake embedding ratio", source)
        self.assertIn("experiment_type: base", source)
        self.assertIn("condition_attribute: none", source)
        self.assertIn("synthetic_strategy: diffusion", source)
        self.assertIn("adversarial_enabled: false", source)
        ...
        self.assertIn("num_workers: 4", source)
        self.assertIn("num_spk: 942", source)
        self.assertIn("save_dir: ${PROJECT}/caarma-output/base-diffusion-2to1-clean", source)
        self.assertIn("title: caarma_base_diffusion_2to1_clean", source)
        self.assertNotIn("data/veri_test.txt", source)
        self.assertNotIn("caarma-gender", source)

    def test_base_diffusion_2to1_tiny_lambda_config_sets_half_fake_fraction(self):
        source = (
            ROOT / "configs" / "base_diffusion_2to1_lambdasyn0025_clean_bridges2.yaml"
        ).read_text()

        self.assertIn("2:1 real-to-fake embedding ratio", source)
        self.assertIn("very low synthetic loss weight", source)
        self.assertIn("experiment_type: base", source)
        self.assertIn("condition_attribute: none", source)
        self.assertIn("synthetic_strategy: diffusion", source)
        self.assertIn("adversarial_enabled: false", source)
        ...
        self.assertIn("validation_split: val", source)
        self.assertIn("diffusion_fake_fraction: 0.5", source)
        self.assertIn("lambda_syn: 0.0025", source)
        self.assertIn("num_workers: 4", source)
        self.assertIn("num_spk: 942", source)
        self.assertIn(
            "save_dir: ${PROJECT}/caarma-output/base-diffusion-2to1-lambdasyn0025-clean",
            source,
        )
        self.assertIn("title: caarma_base_diffusion_2to1_lambdasyn0025_clean", source)
        self.assertNotIn("data/veri_test.txt", source)
        self.assertNotIn("caarma-gender", source)
